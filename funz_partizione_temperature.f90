program funz_partizione
  ...
  ! ==== Allocazione array ====
  ! Calcolare quante iterazioni fare nel ciclo per T, ho bisogno di alloicare il vettore f_vib e tutti quelli che dipendono da T
  ! La temperatura va da 40 a 280 con passo di 20
  num_temperatures = 20 
  ...
  ! Allocare gli array funz_totA e funz_totB in base al numero di temperature ( le ho fissate a 20 ma si può cambiare)
 ...
 ! ==== Chiamo la funzione per calcolare f_vib in un ciclo con temperature ====
  ...
  ! ==== Calcolo della costante per ogni temperatura ====
   DO idT = 1,20
      ...
      ! Scrittura della temperatura e della costante Kp nel file
      write(20, '(F8.0, E12.5)') real(T), K_p(idT)
  ENDDO

  ! Chiusura del file
  close(20)

 ! ==== File no Intestazione (per Python) ====
  open(unit=20, file='risultati_kp_temp_no_intest.dat', status='replace', action='write')
      ! Scrittura della temperatura e della costante Kp nel file
   DO idT = 1,20
      T = 40 + (idT-1) * 20
      write(20, '(F8.0, E12.5)') real(T), K_p(idT)
